...
async def reopen_project_resources(project):
...
async def summarize_last_project_session(project):
...
async def launch_project_environment(project_name):
    await summarize_last_project_session(project_name)
    ...
    for app in apps:
        if app not in APP_PATHS:
            ...
            continue
        await launch_app_by_name(app)
    
    await reopen_project_resources(project_name)
...
# --- App Launcher ---
async def launch_app_by_name(app_name):
...
# --- Scheduler ---
async def schedule_app_launch(app_name, start_time, project_name=None):
    if platform.system() != "Windows":
        print("App launching only supported on Windows.")
        return
    ...
    if delay <= 0:
        print(f"Event is too close or already passed. Launching {app_name or project_name} now.")
        if project_name:
            await launch_project_environment(project_name)
        else:
            await launch_app_by_name(app_name)
    else:
        print(f"Scheduled to launch {app_name or project_name} in {int(delay // 60)}m {int(delay % 60)}s.")
        asyncio.create_task(delayed_launch(delay, app_name, project_name))

async def delayed_launch(delay, app_name, project_name):
...
# --- Event + Task Creation ---
def create_event(title, description="", start_time=None, duration_minutes=60):
    ...
    start_str = start_time.astimezone(pytz.timezone("Europe/Rome")).isoformat()
    end_str = end_time.astimezone(pytz.timezone("Europe/Rome")).isoformat()
    ...
    for event in existing_events:
        ...
        existing_desc = event.get('description', '').strip().lower()

        if existing_title == title.strip().lower() and existing_desc == description.strip().lower():
            print(f"Duplicate event '{title}' already exists in this time range. Skipping creation.")
            return
    ...
    existing_tasks = tasks_service.tasks().list(tasklist='@default').execute().get('items', [])
    for task in existing_tasks:
        task_title = task.get('title', '').strip().lower()
        task_notes = task.get('notes', '').strip().lower()

        if task_title == title.strip().lower() and description.strip().lower() in task_notes:
            print(f"Duplicate task '{title}' already exists. Skipping creation.")
            return
...
async def handle_todays_events():
    creds = get_credentials()
    calendar_service = build('calendar', 'v3', credentials=creds)
    ...
    for event in events:
        ...
        if start_time > datetime.now(start_time.tzinfo):
            ...
            if project != "misc":
                ...
                await schedule_app_launch(None, start_time, project_name=project)
            else:
                app = infer_app_to_launch(event_title)
                print(f"No project match. Inferred app: {app}")
                await schedule_app_launch(app, start_time)
...
async def log_completed_tasks_for_today():
    creds = get_credentials()
